...
//		System.exit(0);
					...
					try
					{
						...
						if(seq_str != null)
						{
							...
							if(basicBlocksFormed.get(pc) == null)
							{
								...
								b.correctFlow(programFlow, lineKey, Long.parseLong(temp_s[0]));
	...
	/*	
		...
		{
			int remaining_here = programFlow.size();
			...
			{
				remaining_here--;
				Map.Entry<BasicBlock, Long> blkTids = programFlow.get(lKey);
				BasicBlock blk = blkTids.getKey();
				Long tid = blkTids.getValue();
				...
				queue.add(blk);
				if(remaining_here % 10000 == 0)
				{
					System.out.println("adding new:"+remaining_here);
				}
				strandMaintainer.addNext(blk, tid);
		...
		//For Speeding the other process only	Collections.sort( list, new Comparator<Map.Entry<String, Long>>()
		//For Speeding the other process only			{
		//For Speeding the other process only			@Override
		//For Speeding the other process only			public int compare( Map.Entry<String, Long> o1, Map.Entry<String, Long> o2 )
		//For Speeding the other process only			{
		//For Speeding the other process only			return -1*((o1.getValue()).compareTo( o2.getValue() ));
		//For Speeding the other process only			}
		//For Speeding the other process only			} );
			double perc = 0.01;
