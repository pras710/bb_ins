...
//		System.exit(0);
	...
	public String getOperand(String s)
	{
		...
		if(s.startsWith("pc = "))
		{
			...
			return GetSrcDest.getMemoryValue(key);
		...
		if(s.indexOf(";")!=-1)
		{
			...
			GetSrcDest.putAVersion("r15", pc_split[0]);
		}
		...
		//	return "";
		//}
		String dat = GetSrcDest.getMemoryValue(s);//memoryMap.get(s);
		if(dat == null)
		{
			if(s.startsWith("["))
			{
				...
				try
				{
					...
					String str = GetSrcDest.getMemoryValue(register);
					address = ""+Long.parseLong(str);
				}
				catch(Exception e)
				{
					e.printStackTrace();
					System.out.println(s+" "+register+" "+GetSrcDest.getMemoryValue(register));
					System.exit(0);
				}
				while(s.length() > s.indexOf("]")+1)
				{
					...
					if(s.startsWith("+"))
					{
						if(s.indexOf("[r")!=-1)
						{
							...
							String  address_2 = GetSrcDest.getMemoryValue(register_2);
							if(address_2 != null)
							{
				...
				try
				{
					...
					//System.out.println("address = "+address);
					dat = GetSrcDest.getMemoryValue(address);//memoryMap.get(address);
					if(dat == null)
					{
	...
	public void manageInstructionParams(String ins, int insCount)
	{
		...
		String opcd = ins.substring(8, 18).trim();
		...
		if(opcd.length() != 4)// && !opcd.startsWith("f"))
		{
			if(myFlag)
			{
				...
				if(insType == null)
				{
					System.out.println(ins+" has no thumb2");
					System.exit(0);
				}
